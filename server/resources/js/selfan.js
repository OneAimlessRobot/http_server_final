...
class Collider{
	...
	constructor(width,height,x,y,planet){
		this.objectos = [];
		this.planet=planet;
		this.x=x;
		this.y=y;
	...
	update(){
		...
		for(let m of this.objectos){
			...
			if(i.pos.vectorzx >= this.x+this.width){
				i.pos=new vectorz(this.x+this.width-1,i.pos.vectorzy)
				i.speed.reflectRightWall();
				i.dampenVector();
			}
			else if(i.pos.vectorzy >= this.y+this.height){
			
				i.pos=new vectorz(i.pos.vectorzx,this.y+this.height-1)
...
class Particle{
	...
	constructor(objecto,mass,charge){
...
return new Particle(objecto,mass,charge);

}
function addLotsOfObjectsToCollider(collider,ammount,size){
	for(let i=0;i<ammount;i++){
		console.log("it added");
		collider.addobject(generateRandMass(generateRandRect(0,0, 500,500,-speeds,-speeds,speeds,speeds,size,getRandomNumber(0,1)),mass,getRandomNumber(-charge,charge)));
		
	}


}
const col=new Collider(500,1200,0,0,new Particle(new Objecto(new vectorz(500,600),new vectorz(0.00001,0.00001),getDiv(50,50,"visible"),0),planetMass,4));
